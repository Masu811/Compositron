...
function gobble_metadata!(parent::xml.XMLElement, metadata::Dict{String, String})
...
function import_hardware_readout!(
    ...
    if isnothing(hardware_id)
        throw(ErrorException("Hardware readout node does not reference a hardware element node"))
    end
    ...
    try
        ...
        throw(ErrorException("Hardware readout node references a hardware element $(hardware_id), but no such element could be found"))
    end
    ...
    if isnothing(set_value_node)
        throw(ErrorException("Hardware readout node does not contain Set-value"))
    end
...
function get_or_parse_detname!(
    ...
    try
        ...
        throw(ErrorException("Could not find node for detector reference '$(det_id)'"))
    end

    detname_node = xml.find_element(det_node, "RadDetectorName")

    if isnothing(detname_node)
        throw(ErrorException("Detector at reference $det_id does not appear to have a name specified"))
    end
...
function get_or_parse_ecal!(
    ...
    try
        ...
        throw(ErrorException("Could not find node for energy calibration reference '$(ecal_id)'"))
    end

    ecal_value_node = xml.find_element(ecal_node, "CoefficientValues")

    if isnothing(ecal_value_node) || xml.content(ecal_value_node) == ""
        throw(ErrorException("Energy calibration node does not appear to contain coefficients"))
    end

    values = xml.content(ecal_value_node) |> data -> split(data, " ") .|> x -> parse(Float64, x)

    if length(values) < 2
        throw(ErrorException("Energy calibration coefficients appear to be in the wrong format"))
    end
...
function parse_spectrum(spectrum_node::xml.XMLElement)::Vector{<:Unsigned}
    ...
    spectrum = channel_data |> data -> split(data, " ") .|> x -> parse(UInt, x)
...
function import_single!(
    ...
    if isnothing(det_id)
        throw(ErrorException("Single Spectrum does not reference a single detector"))
    end
    detname = get_or_parse_detname!(det_id, detectors, parsed_detnames)

    if detname in keys(m.singles)
        throw(ErrorException("Detectors of multiple spectra have the same name '$(detname)'"))
    end
    ...
    s = SingleSpectrum(spectrum, detname, ecal)
...
function parse_detpair(detpair_node::xml.XMLElement)::String
    ...
    if isnothing(name_node) || xml.content(name_node) == ""
        throw(ErrorException("Coincidence Spectrum Node does not contain detector pair name"))
    end
    xml.content(name_node)
end

function get_single_ecal(
    ...
    if isnothing(det_node)
        throw(ErrorException("Coincidence Detector Pair does not reference a Single Detector"))
    end
    det_id = xml.attribute(det_node, "radDetectorInformationReference")
    if isnothing(det_id)
        throw(ErrorException("Coincidence Detector Pair does not reference a first Single Detector"))
    end
    detname = get_or_parse_detname!(det_id, detectors, parsed_detnames)
    try
        s = m.singles[detname]
        ...
        throw(ErrorException("Detector pair is referencing single detector $(detname), but no such detector is known"))
    end
end
...
function parse_hist(
    ...
    if png_filename == ""
        throw(ErrorException("Coincidence Spectrum Node does not reference a PNG file"))
    end
...
function import_coinc!(
    ...
    if isnothing(detpair_id)
        throw(ErrorException("A Coincidence Spectrum Node is not referencing any detector pair"))
    end

    if !(detpair_id in keys(detpairs))
        throw(ErrorException("A Coincidence Spectrum is referencing detector pair $(detpair_id), but no entry for this detector pair was found"))
    end

    detpair_node = detpairs[detpair_id]
    detpair = parse_detpair(detpair_node)

    if detpair in keys(m.coinc)
        throw(ErrorException("Multiple Coincidence Spectra appear to reference the same detector pair $(detpair_id)"))
    end
    ...
    hist = parse_hist(spectrum_node, path)

    c = CoincidenceSpectrum(hist, detpair, ecal)
...
function sort_meas_children!(
    ...
    coinc_nodes = Vector{xml.XMLElement}()
...
function import_n42(filename::String)::DopplerMeasurement
